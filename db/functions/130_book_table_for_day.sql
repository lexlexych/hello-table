-- Бронь конкретного столика на день из портала. Отличие от create_reservation_atomic:
-- там агент ПОДБИРАЕТ столик под компанию на слот slot_minutes, здесь человек указывает
-- столик руками, и столик занят с указанного времени ДО КОНЦА ДНЯ.
--
...
BEGIN
  ...
  -- Буфер buffer_minutes здесь намеренно НЕ применяется, в отличие от
  -- create_reservation_atomic. Он существует ради времени на уборку между двумя
  -- автоматически подобранными посадками; у брони до полуночи он лишь выехал бы
  -- на четверть часа в следующий день и запретил бронировать этот же столик
  -- завтра с 00:00. Проверка совпадает с ограничением reservations_no_overlap
  -- один в один, и функция отвергает ровно то же, что отвергла бы база, — но
  -- своим кодом 45016 вместо 23P01.
  IF EXISTS (
    SELECT 1 FROM reservations res
    WHERE res.table_id = v_table.id
      AND res.status IN ('confirmed','seated')
      AND tstzrange(res.starts_at, res.ends_at) && tstzrange(v_starts, v_ends)
  ) THEN
