...
**Статус.** `check_availability` и `create_reservation` зарегистрированы в голосовом агенте
и напрямую вызывают две RPC под ролью `agent_app`. Их ранее экспортированные workflow n8n
остаются для чата и формуляров. `request_callback` имеет контракт, но временно не
зарегистрирован и не имеет workflow. Остальные контракты пока не определены.
