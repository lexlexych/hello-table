...
В режиме `pipeline` язык берётся из `LanguageTracker`, а перед RPC произносится
детерминированный филлер. В независимом режиме `realtime` отдельного трекера и филлера нет:
...
`invalid_response` формирует агент. В `pipeline` перед RPC он произносит обычный
`filler_checking` на языке сессии; в `realtime` filler-hook не вызывается. При ошибке
используется фраза из `tool_errors` выбранного языка.
