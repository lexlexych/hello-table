...
**Назначение:** возможные времена выдачи для уже собранной корзины. Агент вызывает инструмент
**до** оформления: время готовности считает база из максимального `prep_minutes` позиций и
`pickup_lead_minutes` ресторана, а не модель (PROJECT.md §6.1). Гостю предлагается одно из
возвращённых времён; не более трёх за реплику, поэтому RPC запрашивается ровно с лимитом три.
...
```jsonc
{
  ...
  "slots": [
    ...
    { "ready_at": "2026-08-20T18:15:00.000Z", "date": "2026-08-20", "time": "20:15" }
  ]
}
```
